/*
 *
 * HyStart++ (RFC 9406) — WiFi 802.11g Static  +  IEEE 802.15.4 Mobile
 * =====================================================================
 *
 * ┌─────────────────────────────────────────────────────────────────┐
 ...
 * │    • Pkt/s       : 100 200 300 400 500                          │
 * │    • Coverage ×  : 1  2  3  4  5  (× txRange)                  │
 * └─────────────────────────────────────────────────────────────────┘
 *
 * ┌─────────────────────────────────────────────────────────────────┐
 ...
 * │                   → 2.4 GHz channel (LogDistance loss)          │
 * │                                                                 │
 * │  Flows:  OnOff UDP (rate = pps × pktSize bps)                   │
 ...
 * │    • Flows       : 10 20 30 40 50                               │
 * │    • Pkt/s       : 100 200 300 400 500                          │
 * │    • Speed (m/s) : 5  10  15  20  25   ← mobility parameter     │
 * └─────────────────────────────────────────────────────────────────┘
 *
 ...
 *   cleared at each BeginNewRttRound() call.
 *
 * Build:  place in ns-3 scratch/ and run
...
// ════════════════════════════════════════════════════════════════════════════
//  TcpHyStartPlusPlus  (RFC 9406 + 3-sample weighted-average RTT smoother)
// ════════════════════════════════════════════════════════════════════════════
class TcpHyStartPlusPlus : public TcpLinuxReno
{
  ...
  private:
    ...
    // 3-sample weighted-average smoother (extension)
...
{
    // Only process during initial slow start and CSS phases
    // Once in pure CA, stop HyStart++ algorithm processing
...
TcpHyStartPlusPlus::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bif)
{
    if (phase == HYSTART_SS || phase == HYSTART_CSS)
    {
        ...
        // Only transition to pure CA on loss during startup
        // But don't clear m_isInitialSS yet - let CSS complete its rounds
        phase = HYSTART_CA;
        LogPhaseTransition(Simulator::Now().GetSeconds(),
              ...
              double simTime)
{
    ...
    // OnTime/OffTime to achieve desired pps
    double packetInterval = 1.0 / pps;   // seconds between packets
    double onTime = pktSize * 8.0 / 1e9; // very short burst (approximate)
    // Better: use constant OnTime + OffTime for precise rate
 ...
 *   Idle: ~1.0 mA  →  3.3 mW
 *
 * Energy per node ≈ (P_TX + P_RX) × t_active + P_IDLE × t_idle
 * t_active is approximated from rx/tx bytes ÷ 250 kbps (link rate).
 */
              ...
              double simTime)
{
    ...
    // Use LogDistance propagation loss (suits short-range 802.15.4)
    // Default channel already uses LogDistancePropagationLossModel in
    // LrWpanHelper; we keep it to stay compatible with most ns-3 versions.
    NetDeviceContainer lrDevs = lrWpanHelper.Install(nodes);

    // Associate all devices to a single PAN (ns-3.45 API)
    ...
    // Initial positions: uniform random inside [0, areaSize]²
    ...
    // ── Internet stack (IPv6 only — required for 6LoWPAN) ─────────────────
    ...
    // ── Applications (OnOff UDP over IPv6) ────────────────────────────────
    double appStart = 5.0;
    double appStop = simTime - 1.0;
    uint32_t actual = std::min(flowCount, nodeCount / 2);
    uint32_t offset = std::max(1u, nodeCount / 2);

    // Use requested app rate directly so pps/flow sweeps change offered load.
    ...
    // ── Energy (CC2420 power-state model) ─────────────────────────────────
    // t_active per node ≈ (totalTxBytes + totalRxBytes) / nodeCount / WPAN_RATE
